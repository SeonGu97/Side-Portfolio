...
(23.03.10)
- color_system, palette, color, sentence 추가

- 오늘은 두번째 영역인 color system을 만들었다.
이 영역에는 내가 이 포트폴리오를 만들면서 주로 사용한 머터리얼 컬러들을 나열했다.
대표적으로 그레이와 블루그레이 계열을 사용해서 그 두가지를 채택했다.
그리고 색상을 클릭하면 복사가 되도록 만들고 싶었는데 window.clipboard.writeText 라는 API가 있어서
사용했더니 아주 쉽게 클립보드에 저장이 되었다. 내일은 다음 영역을 만들어야겠다.